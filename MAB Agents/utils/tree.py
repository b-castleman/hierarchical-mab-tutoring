...
class Build_Tree_Data(object):
    def __init__(
        ...
        all_concepts,
        concept_problems,
        all_basic_components,
        problem_components,
        n=1,
        data={"traces": {}, "traces_ngrams": {}},
    ):
...
class Build_Tree(Base_Tree):
    ...
    def _add_child(self, node):
        ...
        if (
            not (node.name in self.all_descendants_names[self.name])
            and node.name != self.name
        ):  # check it is not already a descendant of the subtree it is being inserted into
            if self.verbose:
                print(
                    "add child - self_name: " + self.name + " child_name: " + node.name
                )
    ...
    def _remove_child(self, node):
        ...
        if self.verbose:
            print(
                "remove child - child_name: " + node.name + " self_name: " + self.name
            )
    ...
    def insert_node(self, node_name):
        ...
        if self.name != node_name:
            ...
            if (
                difficulty == 1
            ):  # If the node is harder than you then it belongs somewhere in your subtree
                if len(self.children[self.name]) == 0:
                    ...
                    node = Build_Tree(
                        ...
                        all_descendants_names=self.all_descendants_names,
                        parent=self,
                    ...
                    if (
                        total_harder == 0
                    ):  # if child was not inserted, then it is your child
                        if self.verbose:
    ...
    def return_parents(
        ...
        return {
            key: remove_item(items_list, "Root")
            for key, items_list in self.parents.items()
            if key != "Root"
        }
    ...
    def return_all_ancestors(
        ...
        return {
            key: remove_item(items_list, "Root")
            for key, items_list in self.all_ancestors.items()
            if key != "Root"
        }
