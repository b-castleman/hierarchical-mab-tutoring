...
class ZPDES_Memory_PROBLEM(object):
    ...
    def __init__(
        self, progression_tree, params=DEFAULT_PARAMS, problem_difficulties=None
    ):
        ...
        for concept in all_concepts_list:
            assert (
                concept in problem_difficulties.keys()
            ), "concept does not have defined difficulty"

        assert len(self.problem_difficulties) == len(
            all_concepts_list
        ), "problem difficulty size != concept list size"
        ...
        if "initial_weight_multiplier" not in self.params:
            self.params["initial_weight_multiplier"] = DEFAULT_PARAMS[
        ...
        self.weights = {
            node: self.params["initial_weight"] for node in all_concepts_list
        }
        self.weights_histories = {
            node: [self.params["initial_weight"], self.params["initial_weight"]]
            for node in all_concepts_list
        }
        ...
        self.components_seen_history = {
            component: [] for component in all_components_list
        }
        self.components_memory = {
            component: 0.0001 for component in all_components_list
        }
        ...
        self.m = dict()
        for concept in all_concepts_list:
            self.m[concept] = math.exp(
                -(
                    float(self.problem_difficulties[concept] - 3.0) ** 2
                    / self.params["tau"]
                )
            )
    ...
    def new_problem(self):
        ...
        if self.length_ZPD == 0:
            ...
            if self.memory_concept == None:
                ...
                )  # (float(weights_sum) * gamma/( 1.0 - gamma )) * zeta
                ...
                if self.review_type == memory.REVIEW_TYPES.IN_PROGRESSION:
                    ...
                    for idx, item in enumerate(self.mastered_concepts):
                        weights_memory[idx] = memory_multiplier * max(
                            memory_threshold - self.concepts_memory[item], 0
                        )
                        weights_sum += weights_memory[idx]

                    weights_ZPD = np.hstack([weights_ZPD, weights_memory])
                    concepts_selection = (
                        list(concepts_selection) + self.mastered_concepts
                    )
            ...
            if self.review_type == memory.REVIEW_TYPES.AS_NECESSARY:
                ...
                for key in self.prereqs[self.concept]:
                    if (self.concepts_memory[key] < memory_threshold) and (
                        key not in self.ZPD
                    ):
    ...
    def student_answer_update(self, student_answer_correctness):
        ...
        if self.memory_concept is None and self.concept in self.ZPD:  # If not reviewing
            ...
            ) / float(positiveRewardEnd - positiveRewardStart)

            # Add negative reward last (if there is any yet, otherwise assume previously incorrect)
            reward += (
                -sum(self.correctnesses[concept][negativeRewardStart:negativeRewardEnd])
                / float(history_length)
                if negativeRewardEnd - negativeRewardStart > 0
                else 0
            )
            ...
            if (
                ...
                or (
                    len(self.correctnesses[concept]) >= min_concept_attempts
                    and sum(self.correctnesses[concept][negativeRewardStart:])
                    / float(self.params["history_length"])
                    >= progress_threshold
                )
                or (len(self.correctnesses[concept]) >= max_concept_attempts)
            ):
                self.ZPD.remove(concept)
                self.mastered_concepts.append(concept)

                initial_weight = (
                    self.params["initial_weight"]
                    * self.params["initial_weight_multiplier"]
                )
    ...
    def calculate_concepts_memory(self):
        ...
        self.concepts_memory = memory.get_memory_strengths(
            self.memory_type,
            self.concepts_seen_history,
            current_time_stamp,
            memory_multiplier=self.params["memory_multiplier"],
            base_memory=10000,
        )
        self.components_memory = memory.get_memory_strengths(
            self.memory_type,
            self.components_seen_history,
            current_time_stamp,
            memory_multiplier=self.params["memory_multiplier"],
