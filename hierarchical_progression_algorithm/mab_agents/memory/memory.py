...
def memory_update(
    ...
    if memory_type == MEMORY_TYPES.MCM_AVERAGE:
        ...
        for idx, item_time in enumerate(times_list):
            problem_memory += (
                math.exp((item_time - current_time_stamp) / (float(idx + 1)))
            ) / float(len(times_list))

    elif memory_type == MEMORY_TYPES.MCM_LAST:
        problem_memory = math.exp(
            (times_list[-1] - current_time_stamp) / float(len(times_list))
        )

    elif memory_type == MEMORY_TYPES.LINEAR:
        if (
            len(times_list) * memory_multiplier > current_time_stamp - times_list[-1]
        ...
        if (
            math.exp(len(times_list)) * memory_multiplier
            > current_time_stamp - times_list[-1]
        ):  # doesn't need practice
            problem_memory = 1
        else:  # needs practice
            problem_memory = -1
...
def get_memory_strengths(
    ...
    for key, times_list in problems_history.items():
        if len(times_list) == 0:
            ...
            problems_memory[key] = memory_update(
                memory_type, times_list, current_time_stamp, memory_multiplier
            )
